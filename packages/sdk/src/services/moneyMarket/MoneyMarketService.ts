...
import type { ChainId, EvmContractCall, MoneyMarketConfig } from '../../types.js';
import { poolAbi } from '../../abis/pool.abi.js';
import type { EvmHubProvider, EvmWalletProvider } from '../../entities/index.js';
import { assets, uiPoolDataAbi } from '../../index.js';
...
export class MoneyMarketService {
  ...
  public static supplyData(
    ...
    const assetConfig = assets[spokeChainId][token];
  ...
  public static withdrawData(
    ...
    calls.push(this.encodeWithdraw({ asset: vaultAddress, amount: amount, to: from }, moneyMarketConfig.lendingPool));
  ...
  public static repayData(
    ...
    calls.push(EvmVaultTokenService.encodeDeposit(vaultAddress, assetAddress, amount));
    const translatedAmount = EvmVaultTokenService.translateIncomingDecimals(assetConfig.decimal, amount);
    calls.push(Erc20Service.encodeApprove(vaultAddress, moneyMarketConfig.lendingPool, translatedAmount));
    calls.push(
      this.encodeRepay(
        { asset: vaultAddress, amount: translatedAmount, interestRateMode: 2n, onBehalfOf: to },
        moneyMarketConfig.lendingPool,
      ),
    );
    return encodeContractCalls(calls);
